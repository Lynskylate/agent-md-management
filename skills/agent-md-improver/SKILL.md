...
Audit, evaluate, and improve AGENTS.md files across a codebase to ensure pi has optimal project context.
...
| Pi-specific | `.pi/AGENTS.md` | Pi-specific project context |
| Local overrides | `./.agents.local.md` | Personal/local settings (gitignored, not shared) |
| Global defaults | `~/.pi/agent/AGENTS.md` | User-wide defaults across all projects |
| Package-specific | `./packages/*/AGENTS.md` | Module-level context in monorepos |
| Subdirectory | Any nested location | Feature/domain-specific context |

**Note:** Pi auto-discovers AGENTS.md files in parent directories, making monorepo setups work automatically. See the `init` skill for automatic AGENTS.md generation.
...
- **Use `/revise-agent-md` command**: During a pi session, use `/revise-agent-md` to capture session learnings into AGENTS.md
...
- **Global defaults**: Put user-wide preferences in `~/.pi/agent/AGENTS.md`
- **Use the `init` skill**: Run `/skill:init` to auto-generate a new AGENTS.md from scratch
